/* Due to messing up on Project3A (I created my structs under incorrect assumptions, some functions
 * (such as resize) were not able to work. I have tried again by redoing part A in this file, but have
 * also attached (or pushed) HashTable2.c, which has the old part A struct for the HashTableObject.
 * Anyways, thanks for your time in reading this as it's not quite built. Segfaults in certain odd places */
...
int keyTree(HashTableEntry *entryPtr, char **keysArray, int check)
{
	...
	if (NULL == entryPtr || NULL == entryPtr->key)
	{
		return(-1);
	}
	else
...
int CreateHashTable (HashTablePTR *hashTableHandle, unsigned int initialSize)
{
	...
	{
		// malloc enough size for the hashTable array
...
int InsertEntry (HashTablePTR hashTablePtr, char *key, void *data, void **prevDataHandle)
{
	...
	else
	{
		...
		if (NULL == tempPtr->entry)
		{
			...
			while (1 == hashTablePtr->dynamicBehaviour && useFactor > hashTablePtr->expandUseFactor)
			{
				ResizeTable(hashTablePtr, (unsigned int) ((17/8) * (float) hashTablePtr->range));
				useFactor = GetUseFactor(hashTablePtr);
			}
			return(0);
		}
...
int FindEntry (HashTablePTR hashTablePtr, char *key, void **prevDataHandle)
{
	...
	else
	{
		bucket = SimpleStringHash(key, (unsigned char) hashTablePtr->range);
		tempPtr = (hashTablePtr->bucket)[bucket];
		entryPtr = FindItem(tempPtr->entry, key);
		if (NULL != entryPtr)
		{
...
int DeleteEntry(HashTablePTR hashTablePtr, char *key, void **dataHandle)
{
	...
	else
	{
		...
		useFactor = GetUseFactor(hashTablePtr);
		while (1 == hashTablePtr->dynamicBehaviour && useFactor < hashTablePtr->contractUseFactor && hashTablePtr->range != 1)
		{
			ResizeTable(hashTablePtr, (hashTablePtr->range) / 2 + 1);
			useFactor = GetUseFactor(hashTablePtr);
		}
		return(0);
	}	
