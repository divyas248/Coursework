...
#define FONT_CACHE_SIZE_FOR_ROTATED 1000 // ORIGINALLY 40.
...
void change_button_text(const char *button_name, const char *new_button_text) {
     ...
     * Not a strictly necessary function, since you could instead just 
     * destroy button_name and make a new button.
     */
    int i, bnum;
...
#define ON 0
    ...
    const char* tmpWindow="Window";
    const char* tmpSearch="Search";
    create_button(tmpWindow,tmpSearch,search);
    
    const char *tmpPath="Find Path";
    create_button(tmpWindow,tmpPath,directions);
    
    x11_turn_on_off(ON);
    while (true) {
        
/******************************************************************************************************/             
//DRAW TEXT OVER A POINT OF INTEREST WHEN USER HOVERS MOUSE OVER IT        
/******************************************************************************************************/        
        // Check if mouse if moving


        float mouseX=xscrn_to_world(report.xmotion.x);
        float mouseY=yscrn_to_world(report.xmotion.y);
        
        // Need these to identify the mouse within a grid box
        float xDifferences=abs (tmpLeft-tmpRight)/HOVER_GRID;
        float yDifferences=abs(tmpBot-tmpTop)/HOVER_GRID;
        
        // Identify the grid that the mouse is placed on
        unsigned xMouseTrue= int(floor(abs(mouseX-tmpRight)/xDifferences))%HOVER_GRID;
        unsigned yMouseTrue= int(floor(abs(mouseY-tmpBot)/yDifferences))%HOVER_GRID;
        
        // Get world bounds to use areas in order to fix
        // drawscreen of POIs from triggering at a high level
        t_bound_box tmpBoundBox(minLongitude, minLatitude, maxLongitude, maxLatitude);
        t_bound_box boxCurrent = get_visible_world();
        float ratioZoom = boxCurrent.area()/tmpBoundBox.area();
        
        // Place the world coordinate into a grid
        // xMouseTrue check to ensure spikes don't cause segfaults
        if (xMouseTrue<100 && hoverGrid[xMouseTrue][yMouseTrue]==true){
            //Erase the searched POI
            setSearched=0;
            // Bounce and zoomRatio control to prevent the screen from redrawing multiple times
            if (bounceControl==false && ratioZoom < POI_HOVER_THRESHOLD) {
                drawscreen();
                draw_POI_text(hoverGridID[xMouseTrue][yMouseTrue]);
                bounceControl=true;
            }
        }
        else if (xMouseTrue<100 && hoverGrid[xMouseTrue][yMouseTrue]==false)
            bounceControl=false;
